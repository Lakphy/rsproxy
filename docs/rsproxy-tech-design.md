...
`RSPROXY_LOG` > `RUST_LOG` > `rsproxy=info` 的 filter 优先级和
...
MSRV：最新 stable；edition 2021+。单二进制发布（musl 静态链接可选）。
...
- 协议：HTTP/1.1 + JSON（axum 实现），OpenAPI 文档随代码生成。
...
`cli/help.rs` 集中根命令与 command/subcommand usage；`-h/--help` 在配置加载、
token 发现、daemon/API/platform 操作前拦截。真实二进制测试逐项覆盖全部层级，
证明帮助快速成功退出且不创建 storage 或监听器；未知命令仍返回非零错误。
`cli/completions.rs` 生成 Bash/Zsh/Fish/PowerShell；查询 JSON shape、
...
当前实现已有 445 项常规测试。`tests/cli_help.rs` 以真实二进制和 watchdog 验证
...
错误替代 origin TLS SNI/证书身份的问题。`scripts/verify.sh matrix` 先检查
test list，再逐项执行 34 个 exact owner；CI 必跑该入口。Post-Loop 97 静态验收又
...
合同，阈值边界和缺失字段都会失败。`scripts/verify.sh coverage-report` 使用 llvm-cov 排除测试/
...
```
rsproxy/
├── Cargo.toml                         # workspace 与统一 edition/version
├── .github/workflows/
...
│   │   ├── src/lib.rs                 # 可测试的 CLI/daemon 库入口
│   │   ├── src/logging.rs             # stderr tracing filter、text/JSON 合同
│   │   ├── src/cli/                   # 参数、daemon、CA、规则、trace、系统代理
│   │   │   ├── help.rs                # 副作用前的分层 command/subcommand usage
│   │   │   ├── ca/                    # 证书、存储与系统 trust 操作
│   │   │   └── rules/                 # 分组命令、请求元数据与本地 bench 执行
│   │   ├── src/rule_store/            # ArcSwap 快照、manifest 与原子文件 IO
│   │   ├── src/http/                  # h1 请求头、reader/collect/trailer、响应线
│   │   ├── src/h2/                    # 下游 h2 server、channel body、消息映射与 runtime/IO
│   │   ├── src/proxy.rs               # 代理内核门面与共享依赖
│   │   ├── src/proxy/                 # 接入、转发、路由、WS、body、动作、隧道
│   │   │   ├── server/                # 明文接入、CONNECT、MITM、请求错误
│   │   │   ├── http_flow/             # session、请求体计划、完成/错误归因
│   │   │   ├── h2_bridge/             # 有界请求适配与增量响应 framing
...
│   │   │   ├── transforms/delete.rs   # typed DeleteOp 的 phase orchestration
│   │   │   ├── transforms/delete/body.rs # JSON/form/JSONP nested path 执行
│   │   │   ├── websocket/             # 非阻塞/并发双向转发状态机
│   │   │   ├── connect_tls/           # TLS 握手、观测记录、DNS/TCP/TTFB 时限
│   │   │   ├── tls/                   # TLS 策略、rustls 配置、证书生命周期
│   │   │   └── upstream_response/     # buffered/streaming 响应收尾
│   │   ├── src/control/               # 控制 API、query/value/replay
│   │   ├── src/upstream_h2/            # h2 消息、池、请求 channel 与双向生命周期
│   │   ├── src/upstream_{message,pool}.rs # h2 消息边界与共享 keyed admission
│   │   ├── src/*/tests/               # 可访问私有实现的行为分组单元测试
│   │   │   └── proxy/tests/protocol_matrix/ # WS/mTLS/header/name 真实网络边界
│   │   ├── tests/                     # CLI 黑盒集成测试
...
├── benches/criterion/                 # rules/trace/certificate 微基准与报告
...
├── scripts/verify.sh                 # actions/matrix/bench/quality/resource/package
├── scripts/targets.sh                # criterion/e2e/soak/coverage/regression
...
当前依赖方向：`rsproxy-cli → {rsproxy-rules, rsproxy-trace}`，`rsproxy-trace → rsproxy-rules`，`rsproxy-rules` 不依赖网络层。需要访问私有实现的测试与模块同目录，公开契约测试放在各 crate 的标准 `tests/` 目录；脚本同时约束所有 Rust 文件不超过 500 行、禁止内联测试模块、禁止测试函数脱离专用测试路径、要求每个 crate 保留公开集成测试目录，并约束 CI/performance/fuzz/release workflow 的固定清单与必跑命令。
...
| **M5 打磨发布**（~2 周） | 本机性能专项（§9.3 全指标达标）、fuzz 一轮、长稳、npm/Bun 本机产物、文档 | 本机 v0.1.0 发布资格 |
...
| 风险/问题 | 说明与预案 |
| --- | --- |
| fancy-regex 预算标定 | 已决策：解析期检测特性，普通正则走 `regex`（线性时间），含 backref/lookaround 走 `fancy-regex` + 强制执行预算（回溯步数上限 + 超时熔断，超限按不命中处理并计数告警）。风险在预算参数标定：过松防不住 DoS、过紧误杀，需压测确定默认值并暴露配置 |
| 全新 DSL 的采纳成本 | 已决策不兼容 whistle 语法，老用户需学习新语法。预案：DSL 规范含 whistle 对照表（同场景两种写法并排）、`rules import --from-whistle` 转换器（v2）、`rules test` explain 降低调试门槛 |
| HTTP/2 桥接边界 | trailers 保真与 gRPC 已提升为 M2 验收项（不再是长尾）；剩余长尾为 CONNECT over h2、WS over h2（RFC 8441）、流控窗口极端场景，在集成测试中标注 known-gap，v2 视需求补齐 |
| 后续动态化能力 | 候选方向：① WASM（wasmtime + component model，沙箱好、跨语言）；② 外部进程扩展（unix socket + JSON/gRPC 协议，类 LSP，任意语言可写）。倾向 ②先行（简单、调试友好），①做规则内嵌函数。v2 单独出方案，本期在 `core` 中预留 `Interceptor` trait 扩展点 |
| Windows 支持 | 已有 token-auth named pipe、registry+WinINet system proxy、当前用户 Root-store CA 与 Windows daemon process API；npm target map 与 x64/arm64 原生 runner job 已定义，本轮未在 Windows 运行，不能把结构适配视作目标 OS 验证 |
| 移动端调试（对齐 whistle 常见用法） | 需要 `0.0.0.0` 监听 + 二维码/一键 CA 下发。CA 通过 `http://<ip>:port/rsproxy.crt` 内置端点提供（v1 保留该单一内置 HTTP 端点） |
...
| HTTPS（TLS 1.2 / 1.3） | ✅ | 默认 MITM，`bypass` 透传；origin 信任 WebPKI + 原生系统根 + storage CA，可用 `tls(min=…, ciphers=…)` 约束 |
