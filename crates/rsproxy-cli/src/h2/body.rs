...
pub(super) fn channel_body(receiver: mpsc::Receiver<io::Result<H2BridgeFrame>>) -> H2Body {
    H2ChannelBody { receiver }.boxed()
}

pub(super) fn response_parts(head: H2BridgeHead) -> io::Result<(u16, HeaderMap)> {
    let mut headers = HeaderMap::new();
    append_headers(&mut headers, head.headers, "response")?;
    Ok((head.status, headers))
}

struct H2ChannelBody {
    receiver: mpsc::Receiver<io::Result<H2BridgeFrame>>,
}

impl Body for H2ChannelBody {
    type Data = Bytes;
    type Error = io::Error;
    ...
    fn poll_frame(
        ...
        match Pin::new(&mut self.receiver).poll_recv(context) {
            Poll::Ready(Some(Ok(H2BridgeFrame::Data(data)))) => {
                Poll::Ready(Some(Ok(Frame::data(data))))
            }
            Poll::Ready(Some(Ok(H2BridgeFrame::Trailers(trailers)))) => {
