...
pub(super) fn validate_request_headers(
    ...
    state: &SharedState,
) -> Result<(), H2RequestError> {
    let count = request.headers().len().saturating_add(4);
    if count > state.config.max_header_count {
    ...
    if size > state.config.max_header_size {
        return Err(H2RequestError::new(
            StatusCode::REQUEST_HEADER_FIELDS_TOO_LARGE,
            format!(
...
) -> Result<(RawRequest, String), H2RequestError> {
    let authority = parts
        .uri
        .authority()
...
pub(super) fn request_trailers(
    trailers: &HeaderMap,
    state: &SharedState,
) -> Result<Vec<(String, String)>, H2RequestError> {
    if trailers.len() > state.config.max_header_count {
        return Err(H2RequestError::new(
            StatusCode::REQUEST_HEADER_FIELDS_TOO_LARGE,
            format!(
    ...
    .map_err(|error| {
        ...
        H2RequestError::new(status, error.to_string())
    })?;
    Ok(output)
}
...
pub(super) fn hyper_response(
    head: H2BridgeHead,
    body: mpsc::Receiver<io::Result<H2BridgeFrame>>,
...
pub(super) fn error_response(status: StatusCode, message: &str) -> Response<H2Body> {
...
fn static_body(body: Vec<u8>) -> H2Body {
...
#[derive(Debug)]
pub(super) struct H2RequestError {
