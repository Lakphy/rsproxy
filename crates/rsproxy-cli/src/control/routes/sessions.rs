...
use crate::control::query::query_get;
use crate::{http, json};
use std::io::Write;
...
pub(super) fn list_text<W: Write + ?Sized>(
    ...
    http::write_response(
        ...
        json::sessions_table(&sessions).as_bytes(),
    )
}
...
pub(super) fn follow_live<W: Write + ?Sized>(
    ...
    loop {
        match follow.recv_timeout(Duration::from_millis(heartbeat_ms)) {
            Ok(session) => {
                writeln!(stream, "{}", json::session_summary(session.as_ref()))?;
                stream.flush()?;
            }
...
pub(super) fn spill<W: Write + ?Sized>(stream: &mut W, state: &SharedState) -> std::io::Result<()> {
...
pub(super) fn export_json<W: Write + ?Sized>(
    ...
    respond_json(stream, 200, &json::sessions_json(&sessions))
}
...
pub(super) fn export_har<W: Write + ?Sized>(
    ...
    let sessions = state.trace.list(usize::MAX);
    respond_json(stream, 200, &json::sessions_har(&sessions))
}
...
pub(super) fn get<W: Write + ?Sized>(
    ...
    match state.trace.get(id) {
        Some(session) => respond_json(stream, 200, &json::session_detail(&session)),
