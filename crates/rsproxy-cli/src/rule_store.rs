...
pub(crate) use watch::RuleWatchStatus;
...
impl RuleStore {
    pub(crate) fn validate_name(name: &str) -> Result<(), RuleStoreError> {
        validate_group_name(name)
    }

    pub(crate) fn load(storage: &Path) -> Result<Self, RuleStoreError> {
    ...
    #[cfg(test)]
    pub(crate) fn from_compiled(storage: &Path, compiled: RuleSet) -> Self {
        Self::from_snapshot(
            storage.join("rules"),
            RuleSnapshot {
    ...
    pub(crate) fn snapshot(&self) -> Arc<RuleSnapshot> {
    ...
    pub(crate) fn remove_group(&self, name: &str) -> Result<Arc<RuleSnapshot>, RuleStoreError> {
        validate_group_name(name)?;
        if name == "default" {
...
impl RuleSnapshot {
    ...
    pub(crate) fn group(&self, name: &str) -> Option<&RuleGroup> {
...
impl fmt::Display for RuleStoreError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Watch(message) => write!(f, "rule watcher: {message}"),
        }
    }
}

#[cfg(test)]
#[path = "rule_store/tests.rs"]
mod tests;
