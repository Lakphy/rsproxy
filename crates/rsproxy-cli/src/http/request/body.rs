...
pub(crate) use collect::{read_request_body_all, read_request_body_bounded};
pub(crate) use trailers::validate_request_trailers;

use trailers::{read_crlf_line_limited, read_trailers};

#[derive(Debug)]
pub(crate) enum BoundedRequestBody {
    Complete {
        body: Vec<u8>,
...
pub(crate) enum RequestBodyRead {
    Data(usize),
    End(Vec<(String, String)>),
}

impl RequestBodyReader {
    pub(crate) fn new(framing: RequestBodyFraming) -> Self {
    ...
    pub(crate) fn framing(&self) -> RequestBodyFraming {
        self.framing
    }
