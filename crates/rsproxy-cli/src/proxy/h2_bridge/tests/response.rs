...
) -> (H2BridgeHead, Vec<H2BridgeFrame>, Option<io::Error>) {
...
fn chunked_response_is_incrementally_decoded_with_trailers() {
    ...
    let body = frames
        ...
        .filter_map(|frame| match frame {
            H2BridgeFrame::Data(data) => Some(data.as_ref()),
            H2BridgeFrame::Trailers(_) => None,
        })
        .flatten()
        .copied()
        .collect::<Vec<_>>();
    ...
    assert!(matches!(
        frames.last(),
        Some(H2BridgeFrame::Trailers(trailers))
...
fn incomplete_chunked_response_reports_body_error_after_head() {
    ...
    assert_eq!(head.status, 200);
    assert!(
        matches!(frames.as_slice(), [H2BridgeFrame::Data(data)] if data.as_ref() == b"partial")
    );
    assert!(
        error
            .unwrap()
