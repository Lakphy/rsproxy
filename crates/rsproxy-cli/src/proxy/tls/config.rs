...
pub(in crate::proxy) fn ca_initialized(state: &SharedState) -> bool {
    ...
    ca_dir.join("rsproxy-root-ca.pem").is_file() && ca_dir.join("rsproxy-root-ca-key.pem").is_file()
}
...
pub(in crate::proxy) fn mitm_server_config(
    ...
    let (cert_path, key_path) = ensure_leaf_certificate(&state.config.storage.join("ca"), host)?;
...
fn mitm_root_store(state: &SharedState) -> io::Result<RootCertStore> {
    ...
    let ca_cert = state.config.storage.join("ca/rsproxy-root-ca.pem");
    if ca_cert.is_file() {
