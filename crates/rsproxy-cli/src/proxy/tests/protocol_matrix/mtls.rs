...
fn upstream_mtls_succeeds_with_client_identity_and_fails_without_it_over_real_tls() {
    ...
    let (cert_path, key_path) =
        ensure_leaf_certificate(&success_state.config.storage.join("ca"), HOST).unwrap();
    ...
    assert!(
        missing_body.contains("stage=request_write") || missing_body.contains("stage=tls"),
    ...
    assert!(missing.error.as_deref().is_some_and(|error| {
        error.contains("stage=request_write") || error.contains("stage=tls")
    }));
