...
fn dispatches_status_and_rules_without_changing_route_contracts() {
    ...
    assert_eq!(
        ...
        state.engine.status_snapshot().config.body_buffer_limit
    );
    assert_eq!(
        status_json["config"],
        state
...
fn named_rule_group_routes_publish_one_ordered_snapshot() {
    ...
    .unwrap();
    assert!(state.engine.rules().snapshot().group("override").is_none());
    assert!(!state.options.storage.join("rules/override.rules").exists());
