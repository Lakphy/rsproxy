...
use crate::server::routes::dispatch;
use crate::server::{self as control, router};
use rsproxy_trace::{
...
fn value_ca_trace_and_fallback_routes_cover_success_and_error_contracts() {
    ...
    fs::create_dir_all(state.options.storage.join("values/nested")).unwrap();
    ...
    .unwrap();
    assert!(!state.options.storage.join("values/alpha").exists());

    let mut ca = Vec::new();
    dispatch(&mut ca, &request("GET", "/api/ca/root.pem", &[]), &state).unwrap();
    assert_eq!(status(&ca), 404);
    fs::create_dir_all(state.options.storage.join("ca")).unwrap();
    fs::write(
        state.options.storage.join("ca/rsproxy-root-ca.pem"),
        b"fixture-ca",
    )
    .unwrap();
    let mut ca = Vec::new();
    ...
    fs::remove_dir_all(&state.options.storage).unwrap();
}
...
fn session_routes_list_follow_detail_export_and_missing_spill() {
    ...
    fs::remove_dir_all(&state.options.storage).unwrap_or(());
}
...
fn router_handles_eof_authentication_and_authorized_dispatch() {
    ...
    protected.options.api_token = Some("secret".to_string());
    let unauthorized =
