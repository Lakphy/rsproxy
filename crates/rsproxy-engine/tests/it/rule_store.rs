use rsproxy_engine::{RuleStore, RuleStoreError};
...
#[test]
fn manifestless_v3_rule_files_are_discovered_in_deterministic_order() {
    let storage = temp_storage("legacy");
    let rules_dir = storage.join("rules");
    fs::create_dir_all(&rules_dir).unwrap();
    fs::write(
        rules_dir.join("z.rules"),
        "@language 3\nexample.test status(203)",
    )
    .unwrap();
    fs::write(
...
fn set_disable_and_reload_preserve_group_order_and_state() {
    ...
    store
        .set_group(
            "later",
            "@language 3\nexample.test status(202) @important".to_string(),
        )
        .unwrap();
    assert!(matches!(
...
fn invalid_update_does_not_change_snapshot_or_group_file() {
    ...
    let error = store
        .set_group("default", "@language 3\nexample.test unknown()".to_string())
        .unwrap_err();
...
fn published_snapshots_keep_in_flight_requests_on_their_original_rules() {
    ...
    store
        .set_group(
            "default",
            "@language 3\nexample.test status(202)".to_string(),
        )
        .unwrap();
    assert!(matches!(
