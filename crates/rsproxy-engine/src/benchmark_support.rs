use crate::CaMaterial;
use crate::proxy::tls::{
    ensure_leaf_certificate, generate_leaf_certificate, load_certs, load_certs_from_pem,
    load_private_key,
};
use crate::state::MitmCertCache;
use rcgen::{
...
impl CertificateFixture {
    pub fn create(ca_dir: impl Into<PathBuf>) -> io::Result<Self> {
        ...
        let (certificate, key) = generate_root_ca("rsproxy criterion root")?;
        Ok(Self {
            ca_dir,
            ca_material: CaMaterial::from_pem(certificate, key),
        })
    }

    pub fn issue_leaf(&self, host: &str) -> io::Result<usize> {
        let (certificate, key, chain) = generate_leaf_certificate(&self.ca_material, host)?;
        Ok(certificate.len() + key.len() + chain.len())
    }

    pub fn ensure_leaf(&self, host: &str) -> io::Result<(PathBuf, PathBuf)> {
        ensure_leaf_certificate(&self.ca_dir, &self.ca_material, host)
    }
    ...
    pub fn cached_server_config(&self, host: &str) -> io::Result<CachedServerConfig> {
        ...
        for certificate in load_certs_from_pem(self.ca_material.certificate_pem())? {
...
fn generate_root_ca(common_name: &str) -> io::Result<(String, String)> {
