...
pub(super) struct H2ResponseWriter {
    head_sender: Option<oneshot::Sender<io::Result<DownstreamH2ResponseHead>>>,
    body_sender: Option<mpsc::Sender<io::Result<DownstreamH2ResponseFrame>>>,
    state: ResponseState,
    buffer: Vec<u8>,
    method: String,
...
impl H2ResponseWriter {
        ...
        match self.state {
            ...
            ResponseState::CloseDelimited => {
                if !self.buffer.is_empty() {
                    ...
                    self.send_body(DownstreamH2ResponseFrame::Data(data))?;
                }
                self.close_body();
            ...
            match self.state {
                ...
                ResponseState::ChunkTrailers => {
                    ...
                    if !trailers.is_empty() {
                        self.send_body(DownstreamH2ResponseFrame::Trailers(trailers))?;
                    }
                    self.close_body();
                    self.state = ResponseState::Done;
                }
    ...
    fn parse_head(&mut self, size: usize) -> io::Result<()> {
        ...
        self.send_head(DownstreamH2ResponseHead {
            status: head.status,
            headers,
        })?;
    ...
    fn send_head(&mut self, head: DownstreamH2ResponseHead) -> io::Result<()> {
    ...
    fn send_body(&mut self, frame: DownstreamH2ResponseFrame) -> io::Result<()> {
