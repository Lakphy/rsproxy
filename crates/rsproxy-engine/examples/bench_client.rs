...
#[derive(Debug, Error)]
enum BenchError {
    #[error("invalid benchmark argument: {0}")]
    InvalidArgument(String),
    #[error("benchmark worker panicked")]
    WorkerPanicked,
    #[error("serialize benchmark report: {0}")]
...
impl Config {
    fn parse() -> BenchResult<Self> {
        ...
        let proxy = option(&args, "--proxy")?
            .parse::<SocketAddr>()
            .map_err(|error| BenchError::InvalidArgument(format!("invalid --proxy: {error}")))?;
        let target = option(&args, "--target")?;
        let authority = target
            .strip_prefix("http://")
            .ok_or_else(|| BenchError::InvalidArgument("--target must use http://".to_string()))?
            .split('/')
            .next()
            .filter(|value| !value.is_empty())
            .ok_or_else(|| {
                BenchError::InvalidArgument("--target must include an authority".to_string())
            })?
            .to_string();
...
fn run(config: Config) -> BenchResult<(String, bool)> {
    ...
    Ok((serde_json::to_string(&report)?, failed))
}
...
fn option(args: &[String], name: &str) -> BenchResult<String> {
    args.iter()
        .position(|arg| arg == name)
        .and_then(|index| args.get(index + 1))
        .cloned()
        .ok_or_else(|| BenchError::InvalidArgument(format!("{name} is required")))
}

fn parse_positive(value: &str, name: &str) -> BenchResult<usize> {
    let value = value
        .parse::<usize>()
        .map_err(|_| BenchError::InvalidArgument(format!("{name} must be numeric")))?;
    if value == 0 {
